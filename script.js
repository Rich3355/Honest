let userName = '';
let romanticVideo = null;

document.addEventListener('DOMContentLoaded', function() {
    // Preload the romantic song video
    romanticVideo = document.createElement('video');
    romanticVideo.src = 'Ed Sheeran.mp4';
    romanticVideo.preload = 'auto';
    romanticVideo.loop = true;
    romanticVideo.controls = false;
    romanticVideo.style.display = 'none';
    romanticVideo.addEventListener('loadeddata', () => {
        console.log('Video preloaded and ready to play');
    });
    romanticVideo.addEventListener('error', (e) => {
        console.error('Video preload error:', e);
    });
    document.body.appendChild(romanticVideo);
});
...
document.getElementById('name-form').addEventListener('submit', function(event) {
    ...
    if (name) {
        ...
        document.getElementById('message').textContent = `Jonathan Loves You ${name} — Mmuah 💋`;
...
function playRomanticSong() {
    ...
    if (romanticVideo) {
        romanticVideo.currentTime = 20; // Skip the 20-second intro sound
        romanticVideo.play().then(() => console.log('Video playback started')).catch(e => console.error('Playback failed:', e));
    } else {
        console.error('Video not preloaded');
    }
}
...
document.getElementById('answers-form').addEventListener('submit', async function(event) {
    ...
    if (!likeMost) {
        alert('Please answer what you like most about Jonathan.');
        return;
    }
    if (!annoying) {
        alert('Please answer what makes Jonathan annoying sometimes.');
        return;
    }
    if (!smile) {
        alert('Please answer what’s one thing Jonathan does that makes you smile.');
        return;
    }
    if (!memory) {
        alert('Please answer what’s your favorite memory with Jonathan.');
        return;
    }
    if (!change) {
        alert('Please answer what would you change about Jonathan (if anything).');
        return;
    }
    if (!different) {
        alert('Please answer what makes Jonathan different from other guys.');
        return;
    }
    if (!neverStop) {
        alert('Please answer what’s one thing Jonathan should never stop doing.');
        return;
    }
    if (!threeWords) {
        alert('Please describe Jonathan in three words.');
        return;
    }
...
document.getElementById('relationship-form').addEventListener('submit', async function(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (answer === 'Yes') {
                // Play romantic background song immediately
                playRomanticSong();

                const finalMessage = document.createElement('h2');
                finalMessage.innerHTML = 'You just made Jonathan\'s heart smile 💘';
                ...
                const finalMessage = document.createElement('h2');
                finalMessage.textContent = 'That\'s okay, Jonathan understands. 😊';
